-- Quick fix for generate_monthly_fees function to remove is_active column references
-- This addresses the "column s1.is_active does not exist" error
...
BEGIN
    ...
    -- Loop through all students in the school (removed is_active references)
    FOR v_student IN 
        SELECT 
...
DO $$ BEGIN
    RAISE NOTICE '✅ Fixed generate_monthly_fees function - removed is_active column references';
    RAISE NOTICE '   - Removed s.is_active and s1.is_active conditions';
    RAISE NOTICE '   - Function now works with current students table schema';
    RAISE NOTICE '   - Monthly fee generation should work without column errors';
END $$;