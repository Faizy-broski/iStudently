-- =========================================
-- FIX LIBRARY SETTINGS INTEGRATION
-- This migration updates database functions to use school settings
-- instead of hardcoded values for library policies
-- =========================================
...
DROP FUNCTION IF EXISTS search_students_by_school(UUID, TEXT) CASCADE;
...
-- =========================================
-- 1. UPDATE ELIGIBILITY CHECK TO USE SETTINGS
-- =========================================

-- Enhanced eligibility check that reads from school settings
...
-- =========================================
-- 2. CREATE OPTIMIZED STUDENT SEARCH FUNCTION
-- =========================================

-- Drop existing function if exists
DROP FUNCTION IF EXISTS search_students_for_library(UUID, TEXT, INTEGER) CASCADE;

-- Fast student search function with denormalized data for library operations
-- Uses indexes on profiles table for better performance
...
COMMENT ON FUNCTION search_students_for_library IS 
'Optimized student search for library operations. Returns student info with loan counts. 
Uses indexes on profiles table for fast searching by name or student number.';

-- =========================================
-- 3. CREATE INDEX FOR FASTER STUDENT SEARCH
-- =========================================

-- Create index on profiles for faster name search (if not exists)
CREATE INDEX IF NOT EXISTS idx_profiles_name_search 
ON profiles (school_id, role, is_active, LOWER(first_name), LOWER(last_name));

-- Create index on student_number for faster search
CREATE INDEX IF NOT EXISTS idx_students_number_search 
ON students (school_id, LOWER(student_number));

-- =========================================
-- 4. CREATE HELPER FUNCTION FOR LOAN DUE DATE
-- =========================================

-- Drop existing function if exists
DROP FUNCTION IF EXISTS calculate_loan_due_date(UUID, DATE) CASCADE;

-- Function to calculate due date based on school settings
...
RETURNS DATE AS $$
DECLARE
    v_loan_duration INTEGER := 14; -- default fallback
    v_settings JSONB;
BEGIN
    -- Get school settings for library configuration
    SELECT settings INTO v_settings
    ...
    -- Extract loan_duration_days from settings, use default if not set
...
COMMENT ON FUNCTION calculate_loan_due_date IS 
'Calculates book loan due date based on school library settings (loan_duration_days). 
Defaults to 14 days if not configured.';

-- =========================================
-- 5. CREATE HELPER FUNCTION FOR FINE CALCULATION
-- =========================================

-- Drop existing function if exists
DROP FUNCTION IF EXISTS calculate_overdue_fine(UUID, DATE, DATE) CASCADE;

-- Function to calculate overdue fine based on school settings
...
DECLARE
    v_fine_per_day DECIMAL(10,2) := 0.50; -- default fallback
...
BEGIN
    ...
    -- Extract fine_per_day from settings, use default if not set
...
COMMENT ON FUNCTION calculate_overdue_fine IS 
'Calculates overdue fine based on school library settings (fine_per_day). 
Defaults to $0.50 per day if not configured.';
