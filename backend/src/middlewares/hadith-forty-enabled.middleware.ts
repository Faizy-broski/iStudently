...
// miqat-enabled.middleware.ts — see qirtasi-enabled.middleware.ts's comments
// for the full rationale (campus-row-first lookup, 60s TTL, campus_id
// resolution convention, and the admin-not-pinned-to-one-campus bug already
// hit and fixed twice in this codebase).
const pluginGateCache = new TtlCache<boolean>(60_000)

export async function requireHadithFortyEnabled(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  try {
    ...
    if (pluginActive === undefined) {
      ...
      pluginActive = !!campusResult.data?.active_plugins?.hadith_forty || !!schoolResult.data?.active_plugins?.hadith_forty
      pluginGateCache.set(cacheKey, pluginActive)
    }

    if (!pluginActive) {
      return res.status(403).json({ success: false, error: 'The Forty Hadith module is not enabled for this school' })
    }

    return next()
  } catch (err) {
    console.error('requireHadithFortyEnabled check failed:', err)
    return res.status(500).json({ success: false, error: 'Failed to verify Forty Hadith module status' })
  }
}
