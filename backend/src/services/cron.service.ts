...
class CronService {
    ...
    init() {
        ...
        console.log('   - Monthly Salary Generation: 1st of every month at 2:00 AM')
    ...
    async generateDailyAttendanceForAllSchools() {
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                console.log(`📚 Classes Processed: ${result.data.timetable_entries_processed}`)
                console.log('='.repeat(60))
            ...
            console.error('❌ Critical error in daily attendance generation:', error.message)
        }
    }
        ...
        try {
            ...
            for (const school of schools) {
                try {
                    ...
                    const result = await salaryService.generateBulkSalaries(school.id, month, year)
                    ...
                    console.error(`   ❌ Failed to process ${school.name}:`, schoolError.message)
                    results.push({
                        school_id: school.id,
                        school_name: school.name,
                        success: 0,
            ...
            console.error('❌ Critical error in monthly salary generation:', error.message)
        }
    }
    ...
    async sendPendingApprovalReminders() {
        try {
            ...
            Object.entries(schoolGroups).forEach(([_schoolId, data]: [string, any]) => {
                console.log(`   - ${data.school_name}: ${data.count} pending approval(s)`)
            })
        ...
        try {
            ...
            for (const school of schools) {
                ...
                if (structuresWithoutSalary && structuresWithoutSalary.length > 0) {
                    console.log(`   ⚠️  ${school.name}: ${structuresWithoutSalary.length} missing salary record(s)`)
                    console.log(`   🔄 Regenerating...`)

                    const result = await salaryService.generateBulkSalaries(school.id, month, year)
                    console.log(`   ✅ Regenerated: ${result.success}, ❌ Failed: ${result.failed}`)
                }
            }
    ...
    async manualTriggerMonthlySalary(schoolId?: string) {
        ...
        if (schoolId) {
            console.log(`🔧 Manual trigger: Generating salaries for school ${schoolId}`)
            const result = await salaryService.generateBulkSalaries(schoolId, month, year)
            return result
        } else {
    ...
    async manualTriggerDailyAttendance(date?: string) {
        ...
        console.log(`🔧 Manual trigger: Generating daily attendance for ${targetDate}`)
        await this.generateDailyAttendanceForAllSchools()
