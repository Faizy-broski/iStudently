...
const code = `
  async groupAssignParents(
  ...
  ): Promise<{ updated: number; errors: { parent_id: string; error: string }[] }> {
    ...
    const { data: rows, error: fetchErr } = await supabase
      ...
      .eq('school_id', schoolId)
      .in('id', params.parent_ids)

    if (fetchErr) throw new Error(\`Failed to resolve parents for group assign: \${fetchErr.message}\`)

    const validRows = rows || []
    const validIds = new Set(validRows.map((r: any) => r.id))
    for (const id of params.parent_ids) {
      if (!validIds.has(id)) {
        errors.push({ parent_id: id, error: 'Parent not found or does not belong to this school' })
      }
    }
    ...
    if (customFieldUpdates.length === 0) {
      ...
      for (const row of validRows) {
        ...
        if (updateErr) {
          errors.push({ parent_id: row.id, error: \`Update failed: \${updateErr.message}\` })
        } else {
          updated++
        }
      }
...
`;
fs.appendFileSync('d:/thesocialnexus/studently/backend/src/services/append-parent.ts', code);
// wait, parent service is a class or exported functions? Let's check parent.service.ts
