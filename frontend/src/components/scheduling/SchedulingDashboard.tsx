...
export function SchedulingDashboard() {
  ...
  const t = useTranslations("school.scheduling.dashboard")
  ...
  const cards = useMemo(() => {
    ...
    return [
      ...
      {
        label: t("stat_course_periods"),
        value: stats.total_course_periods,
        icon: BarChart3,
        color: "text-amber-600 bg-amber-50",
      },
      {
        label: t("stat_students_enrolled"),
      ...
      {stats && stats.total_seats > 0 && (
        <div className="rounded-xl border bg-card p-5 shadow-sm">
          ...
          <p className="text-xs text-muted-foreground mt-2">
            {t("utilization_text", { filled: stats.total_filled, total: stats.total_seats })}
            {stats.total_seats - stats.total_filled > 0 && (
              <> &mdash; {t("utilization_seats_available", { count: stats.total_seats - stats.total_filled })}</>
            )}
          </p>
        </div>
      )}
