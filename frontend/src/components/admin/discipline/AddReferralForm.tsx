...
import { getDisciplineFields, createDisciplineReferral, type DisciplineField, type DisciplineTargetType } from '@/lib/api/discipline';
...
export default function AddReferralForm({ targetKind }: { targetKind: DisciplineTargetType }) {
  const t = useTranslations('discipline');
  const { user } = useAuth();
  ...
  async function fetchFields() {
    ...
    try {
      const res = await getDisciplineFields(schoolId, targetKind);
      setFields(res.data ?? []);
    } catch {
