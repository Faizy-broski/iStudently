    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col overflow-hidden p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          <DialogTitle>{t('manageQuestionsForQuiz', { title: quiz.title })}</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col lg:flex-row flex-1 overflow-hidden min-h-0">
          ...
          <div className="flex flex-col w-full lg:w-1/2 border-b lg:border-b-0 lg:border-r overflow-hidden">
            <div className="px-4 py-3 bg-muted/40 border-b flex items-center justify-between shrink-0">
              <span className="text-sm font-semibold">{t('inThisQuiz')}</span>
              <Badge variant="secondary">{mapped?.length ?? 0} {t('questions')}</Badge>
            </div>
                    ...
                    <div key={map.id} className="flex items-start gap-2 rounded-md border bg-background p-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium leading-snug line-clamp-2">
                          {idx + 1}. {map.question?.title ?? t('notAvailable')}
                        </p>
                        <div className="flex items-center gap-2 mt-1">
                          <Badge variant="outline" className="text-xs">
                            {t(`questionTypes.${map.question?.type ?? 'select'}`)}
                          </Badge>
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <Input
                          ...
                          title={t('pointsForQuestion')}
                          onChange={e => setEditingPoints(prev => ({ ...prev, [map.id]: e.target.value }))}
                          onBlur={() => handlePointsBlur(map)}
                          type="number"
                          min={0}
                        />
                        <span className="text-xs text-muted-foreground">{t('pointsSuffix')}</span>
                        <Button
                          variant="ghost"
                          size="icon"
          ...
          <div className="flex flex-col w-full lg:w-1/2 overflow-hidden">
            <div className="px-4 py-3 bg-muted/40 border-b shrink-0">
              <p className="text-sm font-semibold mb-2">{t('questionBank')}</p>
              <div className="flex gap-2">
                ...
                <Select value={filterCategory || NONE} onValueChange={v => setFilterCategory(v === NONE ? '' : v)}>
                  ...
                  <SelectContent>
                    <SelectItem value={NONE}>{t('allCategories')}</SelectItem>
                    {(categories ?? []).map(c => (
                    ...
                    return (
                      ...
                      >
                        ...
                        <Button
                          ...
                          onClick={() => handleAdd(q)}
                        >
                          {alreadyAdded ? t('added') : adding === q.id ? '…' : <><Plus className="w-3 h-3 mr-1" />{t('add')}</>}
                        </Button>
                      </div>
                    )
                  })}
