...
import { Loader2, MessageSquare, Plus, Minus, Save, Palette } from "lucide-react";
import { toast } from "sonner";
import { useAuth } from "@/context/AuthContext";
import { useCampus } from "@/context/CampusContext";
import * as gradesApi from "@/lib/api/grades";
import type { ReportCardCommentCategory, ReportCardComment } from "@/lib/api/grades";
...
type CommentFetcher = (categoryId: string, campusId?: string) => Promise<ReportCardComment[]>;

type CategoryFetcher = (campusId?: string) => Promise<ReportCardCommentCategory[]>;

export type ReportCardCommentsManagerProps = {
  readOnly?: boolean;
  selectedCampusId?: string | null;
  getCategories?: CategoryFetcher;
  getComments?: CommentFetcher;
};

const defaultGetCategories: CategoryFetcher = async (campusId) => {
  const res = await gradesApi.getReportCardCommentCategories(campusId);
  return res.success && res.data ? res.data : [];
};

const defaultGetComments: CommentFetcher = async (categoryId, campusId) => {
  const res = await gradesApi.getReportCardComments(categoryId, campusId);
  return res.success && res.data ? res.data : [];
};

export function ReportCardCommentsManager({
  readOnly = false,
  selectedCampusId,
  ...
  }, [user, selectedCampus?.id, selectedCampusId, getCategories]);
  ...
  }, [user, activeTab, selectedCampus?.id, selectedCampusId, getComments]);
  ...
  const handleSaveComments = async () => {
    ...
    try {
      ...
      for (const row of rows.filter((r) => r._dirty && !r._isNew && !r._deleted)) {
  ...
  const handleSaveCategories = async () => {
    ...
    try {
      ...
      for (const row of catRows.filter((r) => r._dirty && !r._isNew && !r._deleted)) {
  ...
  const hasDirtyComments = rows.some((r) => r._dirty || r._deleted || r._isNew);
  const hasDirtyCats = catRows.some((r) => r._dirty || r._deleted || r._isNew);
          ...
          {loadingCats ? (
            ...
            categories.map((cat) => (
              <TabsTrigger key={cat.id} value={cat.id}>
                ...
                {cat.title}
                <span className="text-xs text-muted-foreground ml-1">({cat.comment_count})</span>
              </TabsTrigger>
            ))
          )}
                            ...
                            <tr key={row.id} className="border-b hover:bg-muted/30">
                              ...
                              <td className="py-2 px-2">
                                <Input
                                  ...
                                  onChange={(e) => updateRow(actualIdx, "sort_order", e.target.value ? parseInt(e.target.value) : null)}
                      ...
                      return (
                        <tr key={row.id} className="border-b hover:bg-muted/30">
                          ...
                          <td className="py-2 px-2">
                            <Input
                              value={row.title}
                              onChange={(e) => updateCatRow(actualIdx, "title", e.target.value)}
                              className="h-8 text-sm"
                              disabled={readOnly}
                            />
                          </td>
                          <td className="py-2 px-2">
                            <input
                              ...
                              onChange={(e) => updateCatRow(actualIdx, "color", e.target.value)}
                          ...
                          <td className="py-2 px-2">
                            <Input
                              type="number"
                              value={row.sort_order ?? ""}
                              onChange={(e) => updateCatRow(actualIdx, "sort_order", e.target.value ? parseInt(e.target.value) : null)}
                              className="h-8 text-sm w-20"
                              disabled={readOnly}
                            />
                          </td>
