...
export default function StaffDetailsPage() {
  ...
  useEffect(() => {
    if (activeTab !== "performance" || !currentStaff?.id) return;
    setPerfLoading(true);
    Promise.all([
      getStaffScore(currentStaff.id),
      getLogs({ staffId: currentStaff.id, limit: 50 }),
  ...
  {(currentStaff.profile?.profile_photo_url || currentStaff.profile?.avatar_url || currentStaff.custom_fields?.personal?.photo) ? (
    // eslint-disable-next-line @next/next/no-img-element
    <img
      src={(currentStaff.profile?.profile_photo_url || currentStaff.profile?.avatar_url || currentStaff.custom_fields?.personal?.photo)!}
      alt={fullName}
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      onClick={() => router.push(`/admin/performance?staff=${currentStaff.id}`)}
                    >
                      View all →
                    </button>
                  </div>
