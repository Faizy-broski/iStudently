...
export default function AttendanceChartPage() {
  ...
  const { data: periodsData } = useSWR(
    schoolId ? ['periods-chart', campusId] : null,
    () => getPeriods(campusId),
    { revalidateOnFocus: false }
  )
  const activePeriods = useMemo(() => (periodsData || []).filter(p => !p.is_break), [periodsData])

  // Attendance codes for legend
  useEffect(() => {
    if (!schoolId) return
    attendanceApi.getAttendanceCodes(schoolId, campusId).then(res => {
  ...
  }, [schoolId, startDateStr, endDateStr, campusId, report, periodId])

  const handleExport = async () => {
    try {
      const blob = await attendanceApi.exportAttendanceSummary(schoolId, startDateStr, endDateStr, campusId)
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `attendance-${report === 'absence' ? 'summary' : 'chart'}-${startDateStr}-to-${endDateStr}.xlsx`
      a.click()
  ...
  // Dynamic extra columns from state_code_breakdown (absence mode)
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {/* ── Student count + download + search bar */}
          {studentCount !== null && (
          ...
          {/* ── Attendance Chart grid (student × dates) */}
              ...
              <Table>
                ...
                <TableBody>
                  {filtered.length === 0 ? (
                    ...
                    const tardy = (bd['Late'] ?? bd['Tardy'] ?? 0)
                    const excused = (bd['Excused Absence'] ?? bd['Excused'] ?? 0)
                    const stateAbs = s.days_absent + s.days_half
                    return (
                      <TableRow key={s.student_id} className="hover:bg-accent/20">
                        <TableCell className="font-medium text-primary">{s.student_name}</TableCell>
                        <TableCell>{s.student_number || '—'}</TableCell>
                        <TableCell>{s.grade_name || '—'}</TableCell>
                        <TableCell className="text-right">{stateAbs}</TableCell>
                        <TableCell className="text-right">{s.days_absent}</TableCell>
                        <TableCell className="text-right">{tardy}</TableCell>
                        <TableCell className="text-right">{excused}</TableCell>
                        {extraCols
                          .filter(k => !['Late', 'Tardy', 'Excused Absence', 'Excused', 'Present', 'Absent'].includes(k))
                          .map(k => (
