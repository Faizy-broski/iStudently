...
export default function AdminIssueBookPage() {
        ...
        try {
            ...
            if (result.success && result.data) {
                const filtered = result.data
                    ...
                    .map(b => ({ id: b.id, title: b.title, author: b.author, available_copies: b.available_copies, copies: [] }));
                setBooks(filtered);
            } else {
                setBooks([]);
            }
    ...
    const handleIssueBook = async () => {
        if (!selectedStudent || !selectedCopy) { toast.error('Please select both student and book copy'); return; }
        if (!user?.access_token) { toast.error('Authentication required'); return; }
        setIsLoading(true);
        try {
            const result = await issueBook({ copyId: selectedCopy.id, student_id: selectedStudent.id, due_date: new Date(dueDate) }, user.access_token);
            if (!result.success) throw new Error(result.error || 'Failed to issue book');
            toast.success('Book issued successfully!');
            setSelectedStudent(null); setSelectedBook(null); setSelectedCopy(null);
            setStudentSearch(""); setBookSearch(""); setStudents([]); setBooks([]);
        } catch (error: any) {
    ...
    return (
        <div className="p-6 space-y-6">
            <div>
                <h1 className="text-3xl font-bold text-[#022172] dark:text-white">Issue Book</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                {students.map((s) => (
                                    <div key={s.id} className="p-3 border rounded-lg hover:bg-gray-50 cursor-pointer"
                                        onClick={() => { setSelectedStudent(s); setStudents([]); setStudentSearch(`${s.first_name} ${s.last_name}`); }}>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5" />Select Book</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <Input
                                    ...
                                    value={bookSearch}
                                    onChange={(e) => { setBookSearch(e.target.value); handleBookSearch(e.target.value); }}
                                    className="pl-9"
                                />
                            </div>
                        </div>
                        {selectedBook && selectedCopy && (
                            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                ...
                                <p className="text-sm text-blue-700 mt-1">{selectedBook.title} by {selectedBook.author}</p>
                                <p className="text-xs text-blue-600 mt-1">Copy: {selectedCopy.accession_number}</p>
                            </div>
                        )}
                        {books.length > 0 && !selectedBook && (
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                {books.map((book) => (
                                    <div key={book.id} className="p-3 border rounded-lg hover:bg-gray-50 cursor-pointer"
                                        onClick={async () => {
                                            setSelectedBook(book); setBooks([]); setBookSearch(book.title);
                                        ...
                                        <Badge variant="secondary" className="mt-1">{book.available_copies} available</Badge>
                                    </div>
                                ))}
                            </div>
                        )}
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {selectedBook.copies.map((copy) => (
                                        <Button key={copy.id} variant={selectedCopy?.id === copy.id ? "default" : "outline"} size="sm" onClick={() => setSelectedCopy(copy)}>
            ...
            {(selectedStudent || selectedBook) && (
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><Calendar className="h-5 w-5" />Issue Details</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        {(!selectedStudent || !selectedCopy) && (
                            <div className="flex items-center gap-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                                ...
                                <span className="text-sm text-yellow-800">Please select both a student and a book copy to proceed.</span>
                            </div>
                        )}
                        <div className="flex justify-end">
                            <Button onClick={handleIssueBook} disabled={!selectedStudent || !selectedCopy || isLoading} className="min-w-32">
