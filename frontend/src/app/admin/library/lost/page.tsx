...
import { Search, BookOpen, User, AlertTriangle, DollarSign } from "lucide-react";
...
export default function AdminLostBooksPage() {
    ...
    const handleMarkLost = async () => {
        if (!selectedLoan) { toast.error('Please select a loan to mark as lost'); return; }
        if (!replacementCost || parseFloat(replacementCost) <= 0) { toast.error('Please enter a valid replacement cost'); return; }
        if (!user?.access_token) { toast.error('Authentication required'); return; }
        setIsLoading(true);
        try {
            ...
            if (!result.success) throw new Error(result.error || 'Failed to mark book as lost');
            toast.success('Book marked as lost successfully!');
            if (selectedStudent) await loadActiveLoans(selectedStudent.id);
            setSelectedLoan(null); setReplacementCost(""); setNotes("");
        } catch (error: any) {
    ...
    useEffect(() => {
        if (selectedLoan?.book_price) setReplacementCost(selectedLoan.book_price.toString());
    }, [selectedLoan]);

    return (
        <div className="p-6 space-y-6">
            <div>
                <h1 className="text-3xl font-bold text-[#022172] dark:text-white">Lost Books</h1>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        {selectedStudent && (
                            <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
                                ...
                                <p className="text-sm text-orange-700 mt-1">{selectedStudent.first_name} {selectedStudent.last_name} ({selectedStudent.admission_number})</p>
                        ...
                        {isLoadingLoans ? (
                            ...
                            <div className="space-y-3">
                                {activeLoans.map((loan) => (
                                    <div key={loan.id}
                                        className={`p-4 border rounded-lg cursor-pointer transition-colors ${selectedLoan?.id === loan.id ? 'border-orange-500 bg-orange-50' : 'hover:bg-gray-50'}`}
                                        ...
                                        <div className="mt-3 text-sm text-muted-foreground">Due: {new Date(loan.due_date).toLocaleDateString()}</div>
                                    </div>
                                ))}
                            </div>
                        )}
            ...
            {selectedLoan && (
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><AlertTriangle className="h-5 w-5" />Mark Book as Lost</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="p-4 bg-gray-50 border rounded-lg">
                            <h4 className="font-medium mb-2">Book Details</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                <div><p><strong>Title:</strong> {selectedLoan.book_title}</p></div>
                                <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <Input id="replacement-cost" type="number" step="0.01" min="0" placeholder="0.00"
                                        value={replacementCost} onChange={(e) => setReplacementCost(e.target.value)} className="pl-9" />
                                </div>
                                <p className="text-xs text-muted-foreground">Original purchase price: ${selectedLoan.book_price || 'N/A'}</p>
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="notes">Notes (Optional)</Label>
                                <Input id="notes" placeholder="Reason for marking as lost..." value={notes} onChange={(e) => setNotes(e.target.value)} />
                            </div>
                        </div>
                        <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                            ...
                            <p className="text-sm text-red-700">Marking a book as lost will:</p>
                        ...
                        <div className="flex justify-end gap-3">
                            ...
                            <Button variant="destructive" onClick={handleMarkLost} disabled={isLoading || !replacementCost} className="min-w-32">
