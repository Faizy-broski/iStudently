...
export default function StudentPortalPollsPage() {
  ...
  const fetchPolls = useCallback(async () => {
    if (!profile?.school_id || !profile?.campus_id) return
  ...
  }, [profile?.school_id, profile?.campus_id])

  useEffect(() => {
    fetchPolls()
  }, [fetchPolls])

  const handleResponseChange = (pollId: string, questionId: string, value: any) => {
    setResponses(prev => ({
      ...prev,
      [pollId]: {
        ...prev[pollId],
  ...
  const handleSubmitPoll = async (poll: portalApi.PortalPoll) => {
    ...
    const missingRequired = poll.questions.filter(q => 
      q.is_required && !pollResponses[q.id]
    )
    ...
    try {
      const formattedResponses: portalApi.PollResponseDTO[] = poll.questions
        ...
        .map(q => {
          ...
          return {
            question_id: q.id,
            answer_text: q.question_type === 'text' ? answer : undefined,
            selected_options: ['single_choice', 'multiple_choice'].includes(q.question_type) 
      ...
      // Update local state to show as voted
              ...
              <CardContent>
                {poll.has_voted ? (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    {poll.show_results && (
                      <p className="text-sm mt-2">Results will be visible after the poll ends.</p>
                    )}
                  </div>
                ) : (
                  <div className="space-y-6">
                    {poll.questions?.map((question, qIndex) => (
                      <div key={question.id} className="space-y-3">
                        ...
                        {question.question_type === 'single_choice' && (
                          <RadioGroup
                            value={responses[poll.id]?.[question.id] || ''}
                            onValueChange={(value) => handleResponseChange(poll.id, question.id, value)}
                          >
                            {(question.options as any[])?.map((option, i) => {
                              ...
                              return (
                                <div key={i} className="flex items-center space-x-2">
                                  <RadioGroupItem value={optionValue} id={`${question.id}-${i}`} />
                        ...
                        {question.question_type === 'multiple_choice' && (
                          <div className="space-y-2">
                            {(question.options as any[])?.map((option, i) => {
                              const optionValue = typeof option === 'string' ? option : option.value
                              const optionLabel = typeof option === 'string' ? option : option.label
                              const selected = responses[poll.id]?.[question.id] || []
                              return (
                                <div key={i} className="flex items-center space-x-2">
                                  <Checkbox
                                    id={`${question.id}-${i}`}
                                    ...
                                    onCheckedChange={(checked) => {
                                      ...
                                      handleResponseChange(poll.id, question.id, newSelected)
                                    }}
                                  />
                                  <Label htmlFor={`${question.id}-${i}`} className="font-normal cursor-pointer">
                                    {optionLabel}
                        ...
                        {question.question_type === 'text' && (
                          <Textarea
                            ...
                            value={responses[poll.id]?.[question.id] || ''}
                            onChange={(e) => handleResponseChange(poll.id, question.id, e.target.value)}
                          />
                        )}
                          ...
                          <div className="flex gap-2">
                            {[1, 2, 3, 4, 5].map((rating) => (
                              <Button
                                key={rating}
                                type="button"
                                variant={responses[poll.id]?.[question.id] === rating ? "default" : "outline"}
                                className="w-10 h-10"
                                onClick={() => handleResponseChange(poll.id, question.id, rating)}
                              >
                                {rating}
                              </Button>
                            ))}
