...
import { useStudentLessonPlans } from '@/hooks/useStudentDashboard'
...
export default function StudentLessonPlanReadPage() {
  const [selectedCpId, setSelectedCpId] = useState<string>('')
  const { coursePeriods, lessons, isLoading, error } = useStudentLessonPlans(selectedCpId || undefined)
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">View published lesson plans for your enrolled courses</p>
      </div>

      {/* Course period filter */}
      <div className="flex items-center gap-3">
        <Select value={selectedCpId || 'all'} onValueChange={v => setSelectedCpId(v === 'all' ? '' : v)}>
          <SelectTrigger className="w-64"><SelectValue placeholder="All Course Periods" /></SelectTrigger>
          <SelectContent>
          ...
          <CardContent className="text-center py-16">
            ...
            <p className="text-muted-foreground text-sm">
              {coursePeriods.length === 0
                ? 'You are not enrolled in any course periods yet.'
                : 'No lesson plans have been published for your courses yet.'}
            </p>
          </CardContent>
        </Card>
      ) : (
