...
export default function ParentDailyTransactionsPage() {
  ...
  const allPayments = useMemo(() =>
    fees.flatMap(f => (f.payments || []).map(p => ({ ...p, fee_name: f.fee_name }))),
    [fees]
  )
                    ...
                    {items.map(p => (
                      <div key={p.id} className="flex items-center justify-between p-3 rounded-lg border bg-card text-sm">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-muted-foreground">{p.payment_reference || (p as any).fee_name || 'No reference'}</span>
                        </div>
