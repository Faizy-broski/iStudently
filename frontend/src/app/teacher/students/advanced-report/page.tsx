...
// Teacher: Advanced Report — identical to admin version.
// Only the breadcrumb link and page title context differ.
...
export default function TeacherAdvancedReportPage() {
  ...
  useEffect(() => {
    const loadCustomFields = async () => {
      try {
        ...
        if (response.success && response.data) setCustomFields(response.data)
  ...
  const toggleCategory = (category: string, isCustom: boolean = false) => {
    ...
    if (allSelected) setSelectedFields(prev => prev.filter(id => !categoryFields.includes(id)))
    else setSelectedFields(prev => [...new Set([...prev, ...categoryFields])])
  }
                ...
                {fields.map(field => {
                  ...
                  return (
                    <div key={fieldId} className="flex items-center space-x-2">
                      <Checkbox id={fieldId} checked={selectedFields.includes(fieldId)} onCheckedChange={() => toggleField(fieldId)} />
