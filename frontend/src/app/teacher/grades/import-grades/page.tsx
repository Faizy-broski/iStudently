...
export default function TeacherImportGradesPage() {
  ...
  // Load course periods — filtered to current teacher's staff_id
  useEffect(() => {
    if (!user) return;
    setLoadingCPs(true);
    Promise.all([
      ...
      .then(([cpRes, mpRes]) => {
        if (cpRes.success && cpRes.data) {
          ...
          const filtered = staffId
            ? cpRes.data.filter((cp) => cp.teacher_id === staffId)
            : cpRes.data;
          setCoursePeriods(filtered);
        }
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await importGradebookGrades({
        ...
        name_columns: studentIdentifier === "name" ? { first_name_col: firstNameCol, last_name_col: lastNameCol } : undefined,
        student_number_col: studentIdentifier === "student_number" ? studentNumberCol : undefined,
        mappings,
        rows: parsedRows,
      });
      if (res.success && res.data) {
                ...
                {loadingAssignments ? (
                  ...
                  <div className="space-y-4">
                    {assignments.map((a) => (
                      <div key={a.id}>
                        <Select
                          value={assignmentMappings.get(a.id) !== undefined ? String(assignmentMappings.get(a.id)) : ""}
                          onValueChange={(v) => {
