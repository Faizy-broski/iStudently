...
import { listReportsForTeacher, type InspectionReport } from '@/lib/api/inspection-report'

export default function TeacherReportsPage() {
  const t = useTranslations('inspections.reports')
  const [reports, setReports] = useState<Array<InspectionReport & { school: { id: string; name: string } }>>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    listReportsForTeacher().then((res) => {
      setReports(res.data || [])
      setLoading(false)
    })
  }, [])
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="divide-y divide-gray-100">
              {reports.map((r) => (
                <Link key={r.id} href={`/teacher/inspections/reports/${r.id}/sign`} className="flex items-center justify-between gap-3 p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-gray-900 truncate">{r.school?.name}</div>
