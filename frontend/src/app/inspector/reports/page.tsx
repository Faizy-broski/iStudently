...
import { listReportsForInspector, type InspectionReport } from '@/lib/api/inspection-report'

type Row = InspectionReport & { school: { id: string; name: string }; teacher: { id: string; first_name: string; last_name: string } }
...
export default function InspectorReportsPage() {
  ...
  useEffect(() => {
    listReportsForInspector().then((res) => {
      setReports(res.data || [])
      setLoading(false)
    })
  }, [])
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">{t('inspector_list_subtitle')}</p>
      </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="divide-y divide-gray-100">
              {reports.map((r) => (
                <Link key={r.id} href={`/inspector/visits/${r.visit_id}/report`} className="flex items-center justify-between gap-3 p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-gray-500 truncate">{r.school?.name}</div>
                    </div>
                  </div>
