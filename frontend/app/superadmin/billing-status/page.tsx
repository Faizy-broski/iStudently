...
export default function BillingStatusPage() {
  ...
  const fetchBillingRecords = async () => {
    try {
      setLoading(true);
      const data = await billingRecordsApi.getAll();
      ...
      if (error.message?.includes("billing_records") || error.message?.includes("relation")) {
        toast.error("Billing tables not set up", {
          description: "Please run the database migration to create billing tables. See BILLING_SETUP.md",
          duration: 8000,
        });
      } else {
  ...
  const handleMarkAsPaid = async () => {
    ...
    try {
      ...
      const result = await billingRecordsApi.markAsPaid(selectedRecord.id);
  ...
  const handleCreateBilling = async (data: Omit<BillingRecord, "id" | "school_name" | "subscription_plan" | "invoice_number" | "created_at">) => {
    try {
      const newRecord = await billingRecordsApi.create(data);
  ...
  const handleUpdateBilling = async (data: Omit<BillingRecord, "id" | "school_name" | "subscription_plan" | "invoice_number" | "created_at">) => {
    ...
    try {
      ...
      const updatedRecord = await billingRecordsApi.update(editingRecord.id, updateData);
  ...
  const handleDeleteBilling = async () => {
    ...
    try {
      ...
      setBillingRecords(prev => prev.filter(record => record.id !== selectedRecord.id));
  ...
  const handleCreatePlan = async (data: Omit<BillingPlan, "id" | "created_at">) => {
    try {
      const newPlan = await billingPlansApi.create(data);
      setBillingPlans(prev => [newPlan, ...prev]);
  ...
  const handleDeletePlan = async () => {
    ...
    try {
      ...
      setBillingPlans(prev => prev.filter(plan => plan.id !== selectedPlan.id));
  ...
  const stats = {
    ...
    overdueCount: billingRecords.filter(r => r.payment_status === "overdue").length,
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="gradient-orange text-white hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
          <CardContent className="p-6">
            ...
            <div className="text-3xl font-bold">${stats.pendingRevenue.toLocaleString()}</div>
        ...
        <TabsContent value="billing-records" className="space-y-4 mt-6">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              variant="outline"
              className="gap-2 hover-gradient-blue transition-all duration-300"
            >
